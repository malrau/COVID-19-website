...
```{r, echo=FALSE}
data_table <- merge(
  csse_confirmed_table %>% subset(is.na(`Province/State`)) %>% rename(Confirmed = "Cases"),
  csse_deaths_table %>% subset(is.na(`Province/State`)) %>% rename(Deaths = "Cases"),
  all = TRUE
) %>% select(-`Province/State`)
data_table <- merge(
  data_table,
  csse_recovered_table %>% subset(is.na(`Province/State`)) %>% rename(Recovered = "Cases"),
  all = TRUE
) %>% select(-`Province/State`)
